...
var Mesonet = (function () {
    ...
    Mesonet.prototype.parseApiArgs = function () {
    ...
    Mesonet.prototype._parseURL = function (urlString) {
        var result = {};
     ...
     */
    Mesonet.prototype.setDevServer = function (bool, port) {
     ...
     * @returns {boolean}
     */
    Mesonet.prototype.isDevServer = function () {
        return this.state.http.devServer;
    };

    /**
     * Set the API service to use.
     * @param {string} apiService - Mesonet API service
     */
    Mesonet.prototype.setService = function (apiService) {
        this.state.apiService = apiService;
    };

    Mesonet.prototype.sendTelemetry = function (text) {
        this.store.telemetry.push(text);
    };

    Mesonet.prototype.getTelemetry = function () {
        return this.store.telemetry;
    };

    Mesonet.prototype.fetchQCTypes = function (bool) {
        this.state.http.getQCTypes = bool;
    };

    Mesonet.prototype.fetchVariables = function (bool) {
        this.state.http.getVariables = bool;
    };

    Mesonet.prototype._mergeObject = function(base, top) {
        var merged = {};
        var key;
     ...
     * @example
     */
    Mesonet.prototype.apiDateToEpoch = function (apiDate) {
        var a = '';
    ...
    Mesonet.prototype.parseAPITime = function (t, local) {
        ...
        var _t = new Date(this._parseAPITime(t, local) * 1000).toISOString();
     ...
     * @returns {promise}
     */
    Mesonet.prototype._fetchHandler = function (args) {
        ...
        var baseURL = '//api.mesowest.net/v2/';
        if (props.devServer) {
            baseURL = '//dev2.mesowest.net:' + this.state.http.devServerPort + '/';
    ...
    // Fetches the Mesonet webservices
    Mesonet.prototype._httpGet = function (args) {
        ...
        try {
            $.ajax({
                ...
                complete: function (response) {
                    ...
                    if (response.SUMMARY.RESPONSE_CODE !== 1) {
                        _this.sendTelemetry(
                            'Error code issued from Mesonet API: ' + response.SUMMARY.RESPONSE_MESSAGE
                        );
            ...
            _this.sendTelemetry('Failed to connect to Mesonet API.');
        }
        return deferred.promise();
    ...
    Mesonet.prototype._haveResponse = function (args, obj) {

        // Check the API reponse code
        this.store.status = obj.SUMMARY.RESPONSE_CODE;
        ...
        if (args.service === this.state.api.serviceURLs.Variables) {
            ...
            this.store.sensors = {};
            this.store.sensors.sensorByVid = a;
            this.store.sensors.sensorByName = b;
            return;
        }
        ...
        if (args.service === this.state.api.serviceURLs.QCTypes) {
            ...
            this.store.qcTypes = a;
            return;
        }

        // First, populate with things we don't need to change.
        this.store.info.nObjects = obj.SUMMARY.NUMBER_OF_OBJECTS;
        this.store.info.responseMessage = obj.SUMMARY.RESPONSE_MESSAGE;

        this.store.station = obj.STATION;
        this.store.units = obj.UNITS;
        this.store.tableOfContents = this._generateTOC(obj);
        this.store.tableOfContentsIndex = this._generateTOCIndex(obj);

        if (typeof obj.UIMODE !== 'undefined') { this.store.ui = this._parseUiMode(obj); }
    };

    Mesonet.prototype._generateTOC = function (obj) {
        var toc = {};
    ...
    Mesonet.prototype._generateTOCIndex = function (obj) {
        var toc = {};
        obj.STATION.map(function (k, i) {
            toc[i] = obj.STATION[i].STID;
        });
        return toc;
    };

    Mesonet.prototype._parseUiMode = function (obj) {
        var result = {};
