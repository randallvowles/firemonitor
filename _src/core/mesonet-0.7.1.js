...
var Mesonet = (function() {
    ...
    Mesonet.prototype._apiBrokerAsyncManager = function(args) {
        ...
        if (this.urlArgs.dev !== undefined) {
            ...
            ws.base_url = "http://api.mesowest.net/v2/";
        }
     ...
     * UI Table APIs
     */
    Mesonet.prototype.tabTable = function(args) {
        __this = this;
        $.when(activeRequest).done(function() {
            __this._tabTableEmitter(args);
        });
        return true;
    };

    // Mesonet.prototype.makeTable = function(args) {
    //     _makeGenericTable(args);
    //     return true;
    // };
    /**
     * Creates a HTML table within the DOM tree from the internal data storage.
     */
    Mesonet.prototype._tabTableEmitter = function(args) {

        var __r = __this.response;
        var __c = __this.config.table;

        // Validate arguments
        if (typeof args === "undefined") {
            console.log("No arguments defined.  #nghyt");
            return false;
        } else if (
            typeof args.options === "undefined" ||
            typeof args.options.container_id === "undefined"
        ) {
            console.log("No container ID.  #eofa9");
            return false;
        } else if (typeof args.options.table_id === "undefined") {
            args.options.table_id = "timeseries-table";
        }

        if (
            typeof __r.summary === "undefined" ||
            __r.summary.RESPONSE_CODE !== 1
        ) {
            // NOTE: This is usually caught upstream!
            _createDOMElement({
                element: "P",
                attribute: {
                    parent: args.options.container_id
                },
                inner: __r.summary.RESPONSE_MESSAGE
            });

            console.log("No data avaiable to generate table.  #3luv5");
            console.log(__r.response.summary);
            return false;
        }
        ...
        if (typeof document.getElementById(args.options.table_id) !== "undefined") {
            try {
                ...
                document.getElementById(args.options.qc_container).innerHTML = "";
            } catch (err) {
                console.log("Can not connect to " + args.options.table_id + " " + "in the DOM tree. #91os6");
            }
        }

        var idx = 0; // for now, will be surfaced later
        var __obs = __r.station[idx].OBSERVATIONS;
        var i, j, lj, li, _i;

        // Create rank lookup table
        // !! Propose this be moved to the API Broker and surfaced in 
        //    internal storage
        // Reorder the variable stack based on rank
        var rankedSensors = ["date_time"];
        var rankedLongNames = ["Time"]; // Might surface this later too?
        var rankedUnits = [""];
        var qcStack = [];
        var sensorNumber = 0;
        var s, qcFlag;

        // Organize the sensor stacks and make easy to use linear arrays.
        li = __r.sensor.metadata.rank.length;
        for (i = 0; i < li; i++) {
            _i = __r.sensor.metadata.rank[i];
            lj = __r.sensor.stack[idx].length;

            for (j = 0; j < lj; j++) {
                // @todo: why is this being flag by lint?
                if (__r.sensor.stack[idx][j].match(/.+?(?=\_set\_.)/) == _i) {
                    sensorNumber = __r.sensor.stack[idx][j].match(/(?=\_set\_.)?\d+/);
                    if (sensorNumber > 1) {
                        rankedSensors.push(__r.sensor.stack[idx][j]);
                        rankedUnits.push(__r.sensor.units[idx][_i]);
                        rankedLongNames.push(__r.sensor.metadata.meta[_i].long_name +
                            " (" + sensorNumber + ")");
                    } else {
                        rankedSensors.push(__r.sensor.stack[idx][j]);
                        rankedUnits.push(__r.sensor.units[idx][_i]);
        ...
        // Because some sensors are not in the metadata listing...
        if (rankedSensors.lenth !== lj) {
            //console.log("looking for lost sensors...");
            for (i = 0; i < lj; i++) {
                _i = __r.sensor.stack[idx][i];
                if (!_has(_i, rankedSensors)) {

                    rankedSensors.push(_i);
                    rankedUnits.push(__r.sensor.units[idx][_i.match(/.+?(?=\_set\_.)/)]);

                    // Attempt to format the sensor name into someething useable.
                    // Note that the `^` is to indicate the value is not in
                    // the metadata table.
                    s = _i.match(/.+?(?=\_set\_.)/)[0].replace(/\_/gi, ' ');
                    rankedLongNames.push(__toTitleCase(s) + "^");
                }
            }
        }
        ...
        _createDOMElement({
            element: "TABLE",
            attribute: {
                id: args.options.table_id,
                parent: args.options.container_id,
                class: args.options.table_class
            }
        });
        ...
        for (i = 0; i < li; i++) {
            ...
            if (i === 0) {
                ...
                document.getElementById(args.options.table_id)
                    .getElementsByTagName("thead")[0]
                    .appendChild(document.createElement("TR"));

                el = document.getElementById(args.options.table_id)
                    .getElementsByTagName('tr')[i];

                for (j = 0; j < lj; j++) {
                    el.appendChild(document.createElement("TH")).appendChild(
                        document.createTextNode(rankedLongNames[j])
                    );
                    // Debug
                    // el.appendChild(document.createElement("TH"))
                    //     .appendChild(document.createTextNode(rankedSensors[j]));
                }
            } else {
                document.getElementById(args.options.table_id)
                    ...
                    .appendChild(document.createElement("TR"));
                if (
                    (__r.station[idx].OBSERVATIONS[rankedSensors[0]][i - 1] >
                        __r.station[idx].OBSERVATIONS[rankedSensors[0]][i]) &&
                    (typeof args.options.day_change_class !== "undefined")
                ) {
                    document.getElementById(args.options.table_id)
                        .getElementsByTagName('tr')[i]
                        .setAttribute("class", args.options.day_change_class);
                }

                for (j = 0; j < lj; j++) {

                    //__r.station[idx].OBSERVATIONS[rankedSensors[j]][i];
                    s = __r.station[idx].OBSERVATIONS[rankedSensors[j]][i - 1];
                    if (s === null) {
                        s = "";
                    }

                    // Check to see if there is a qc flag associated, if not then
                    // just make a regular `TD` and add it to the table.
                    if (__r.qc.active[idx] && _has(rankedSensors[j], __r.qc.stack[idx])) {

                        qcFlag = __r.qc.flags[idx][rankedSensors[j]][i - 1];
                        if (qcFlag !== null) {
                            s = s + " (" + qcFlag + ")";

                            el = _createDOMElement({
                                element: "TD",
                                attribute: {
                                    class: args.options.qc_flag_class
                                },
                                inner: s
                            });
                            qcStack.push(__r.qc.flags[idx][rankedSensors[j]][i - 1]);
                        } else {
                            s = __r.station[idx].OBSERVATIONS[rankedSensors[j]][i - 1];
                            el = _createDOMElement({
                                element: "TD",
                    ...
                    document.getElementById(args.options.table_id)
                        .getElementsByTagName('tr')[i].appendChild(el);
                }
            }
        }
        ...
        if (
            typeof args.options.show_units !== "undefined" &&
            args.options.show_units
        ) {
            //var row = document.getElementById(args.options.table_id).insertRow(1);
            var row = document.getElementById(args.options.table_id)
                .getElementsByTagName("tbody")[0]
                .insertRow(0);

            li = rankedUnits.length;
            for (i = 0; i < li; i++) {
                var cell = row.insertCell(i);
                cell.innerHTML = rankedUnits[i];
                //cell.innerHTML = i;
            }
        }

        // Append a UL node to the DOM for the QC Flags    
        if (typeof args.options.qc_container !== "undefined" && __r.qc.active[idx]) {
            var qcStackLinear = [];
            // The `qcStack` array is 2D, need to flatten
            for (i = 0; i < qcStack.length; i++) {
                for (j = 0; j < qcStack[i].length; j++) {
                    qcStackLinear.push(qcStack[i][j]);
                }
            }
            qcStackLinear = this._sortUnique(qcStackLinear);

            _createDOMElement({
                element: "UL",
                attribute: {
                    parent: args.options.qc_container,
                    id: "qcListing111"
                }
            });

            for (i = 0; i < qcStackLinear.length; i++) {
                _createDOMElement({
                    element: "LI",
                    attribute: {
                        parent: "qcListing111"
                    },
                    inner: "(" + __r.qc.metadata[qcStackLinear[i]].ID + ")  " +
                        __r.qc.metadata[qcStackLinear[i]].NAME
                });
            }
         ...
         */
        function _createDOMElement(args) {
            ...
            // @fixme: There is a bug here and I don't know what it is yet.
            if (typeof args.inner !== "undefined") {
                ...
                //     }
                // }
                el.appendChild(document.createTextNode(args.inner));
            }
            ...
            if (typeof args.attribute.parent !== "undefined") {
                try {
                    ...
                    console.log("Can not connect to " + args.attribute.parent + " in the DOM tree.  #lskpx");
                }
            }
